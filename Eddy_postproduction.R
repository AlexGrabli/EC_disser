...
# join_for_gapfilling - Fill gap by date ----------------------------------
...
join_for_gapfilling = function(data_,biometdata_){
  ...
  print(biometdata_)
  biometdata_[['TIMESTAMP']] = 
    as.POSIXct(as.POSIXlt(biometdata_[['TIMESTAMP']], 
                          format="%Y-%m-%d %H:%M:%S", 
                          tz="MSK"))
...
daily_data = function(Data){
  ...
  #add posix dates
  return(cbind(daily_means, Reco, GPP, Reco_sum, GPP_sum, daily_sums, daily_errors, daily_cumsums_g, NA_count,NA_marker ))
...
PlotDiurnal = function(DataList, startM=1, endM=12, riseplot = "TRUE", title_text="NEE for two towers, diurnal") {
  pd = position_dodge(.1)
  shape_list = as.factor(c(1,17,2,19,15,31))
  DP = ggplot()
  linetypes=c( "solid", "dashed", "dotted", "dotdash", "longdash", "twodash")
  for (n in 1:length(DataList)) {
    #finding mean rise hour per month
    rise = as.vector(by(DataList[[n]]$dt[,c("hour","SolElev","Doy")], DataList[[n]]$dt$month_number, function(x) mean(by(x, x$Doy, function(y) return(y$hour[ min(which(y$SolElev>0)) ])))))
    rise=rise-2
    #finding mean down hour per month
    down = as.vector(by(DataList[[n]]$dt[,c("hour","SolElev","Doy")], DataList[[n]]$dt$month_number, function(x) mean(by(x, x$Doy, function(y) return(y$hour[ max(which(y$SolElev>0)) ])))))
    down=down-2
    #and putting it with it's month to df
    hour_months = as.numeric(levels(as.factor(DataList[[n]]$hourly$NEE_f$hour_months)))
    downrisedf = data.frame(hour_months,rise,down)
    ...
    months = sort(months)
    print(sort(months))
    DataList[[n]]$hourly$NEE_f = cbind(DataList[[n]]$hourly$NEE_f, month = months[DataList[[n]]$hourly$NEE_f$hour_months])
  ...
  DP = DP +scale_x_continuous(breaks = round(seq(0,24, by = 4),1))
  if((endM-startM>3)|(endM-startM == 2))  {DP = DP + facet_wrap(~month, ncol =3)} else
  {DP = DP + facet_wrap(~month, ncol =2)}
...
PlotBiomet = function(DataList, filled=FALSE, startDoy, endDoy) {
  ...
  for(n in 1:length(DataList)) {
    ...
    if (!is.null( plot_data$PAR_Den_Avg)) {
      ...
      Gr_PAR = Gr_PAR + geom_line(data =  plot_data , aes(x=Doy-3, y=ma(PAR_Den_Avg)),linetype=linetypes[n], size=.8)
    ...
    if (!is.null( plot_data$Tsoil_f)) {
      Gr_Tsoil = Gr_Tsoil + geom_line(data = plot_data , aes(x=Doy-3, y=ma(Tsoil_f)),linetype=linetypes[n], size=.8)
    ...
    if (!is.null( plot_data$SWC_1)) {
      Gr_water = Gr_water + geom_line(data =  plot_data, aes(x=Doy-3, y=ma(SWC_1*100)),linetype=linetypes[n], size=.8)
    ...
    if (!is.null( plot_data$Rain_mm_Tot_sums) || !is.null( plot_data$P_tot_sums) ){
      Gr_water = Gr_water + geom_rect(data =  plot_data, aes(x=Doy, xmin=Doy-.5, xmax=Doy+.5, ymax = Rain_mm_Tot_sums, y=Rain_mm_Tot_sums, ymin=0, xmin=2), fill=color_list[n], position=pd,linetype = 1, size = .1,alpha = .41, color = "black")
    }
  }
...
PlotFluxSep = function(DataList,filled = FALSE, startDoy=1, endDoy=12) {
  ...
  for(n in 1:length(DataList)) {
    ...
    pd = position_dodge(.1*n)
    Gr_NEE = Gr_NEE + geom_line(data = plot_data, aes(x=Doy-3, y=ma(NEE_f_sums* 12*18 /10000)), size=.8, position=pd, linetype=linetypes[n])
   ...
   if(any(names(DataList[[n]]$daily)=="Rs_n")){
    ...
    Gr_Reco = Gr_Reco + geom_line(data = plot_data, aes(x=Doy-3, y=ma(Reco * 12*18 /10000)), size=.8, position=pd, linetype=linetypes[n])
    ...
    Gr_GPP = Gr_GPP +geom_line(data = plot_data, aes(x=Doy-3, y=ma(GPP * 12*18 /10000)), size=.8, position=pd, linetype=linetypes[n])
...
PlotGPPVegetationAligned = function (DataList,filled = FALSE, fieldnames = c("M", "K")) {
  pd = position_dodge(.1)
  linetypes=c("solid", "dashed", "dotted", "dotdash", "longdash", "twodash")
  shape_list = as.factor(c(15,21,17,19))
  Gr_GPP = ggplot()
  ...
  for(n in 1:length(DataList)) {
    ...
    maxDoy = c(maxDoy, max(plot_data$vegetation_day, na.rm=TRUE))
    minDoy = c(minDoy, min(plot_data$vegetation_day, na.rm = TRUE))
    levels = levels(as.factor(plot_data$phase))
    pd = position_dodge(.1*n)
    Gr_GPP = Gr_GPP + geom_line(data = plot_data, aes(x=vegetation_day-3, y=ma(GPP* 12*18 /10000)), size=.8, position=pd, linetype=linetypes[n])
    Gr_GPP = Gr_GPP + geom_point(data = plot_data , aes(x=vegetation_day, y=GPP* 12*18 /10000),position=pd,size=2, shape=shape_list[n], fill=n,alpha=.5)
    phases_lines = data.frame(levels, veg_day = as.vector(by(plot_data, as.factor(plot_data$phase), function(x) min(x$vegetation_day))), label_height = max(plot_data$GPP, na.rm = TRUE)*(.9)*12*18 /10000)
    
    phases_lines = cbind(phases_lines[with(phases_lines, order(veg_day)), ], 
                         letter = c("a",letters[1:length(levels)-1]),
                         field = rep(fieldnames[n],times = length(phases_lines[,1])))
    print(phases_lines)
    phases_lines$levels = c(NA,as.character(phases_lines$levels[-(length(phases_lines$levels))]))
    #phases_lines=phases_lines[-(length(phases_lines[,1])),]
    phases_lines = phases_lines[-1,]
    bar_length = phases_lines$veg_day
    bar_length[1] = bar_length[1] + 1
    bar_length[2:length(bar_length)] = bar_length[2:length(bar_length)] - bar_length[1:(length(bar_length-1))]
    bar_length[length(bar_length)] = max(plot_data$vegetation_day, na.rm=TRUE)-sum(bar_length[1:(length(bar_length)-1)])
    # Last stage length is calculated to acwardly but should work
    phases_lines$veg_day[length(phases_lines$veg_day)] = phases_lines$veg_day[length(phases_lines$veg_day)-1] + bar_length[length(bar_length)]
    text_pos = cumsum(phases_lines$veg_day)-phases_lines$veg_day*.5
    #bar_length = c(phases_lines$veg_day,0) - c(0, phases_lines$veg_day)
    #print(bar_length)
    phases_lines = cbind(phases_lines, text_pos, bar_length)
    if (n==1){
  ...
  Gr_bars = ggplot(bar_legend, aes(x = field, y = bar_length))
  
  #Gr_bars = Gr_bars + geom_bar( stat = "identity", color="black", aes(width = .9, stat = "identity"),position="fill")
  Gr_bars = Gr_bars + geom_bar(aes(fill=levels), stat="identity")
  Gr_bars = Gr_bars + scale_fill_brewer(palette="Paired")
  Gr_bars = Gr_bars + geom_text(aes(label=letter, y = text_pos), color="black",size=4,position = position_dodge(0.1)) 
  Gr_bars = Gr_bars + coord_flip()  + scale_fill_few()  + scale_y_continuous(limits = c(0, 160)) 
  Gr_bars = Gr_bars + theme_few(base_size = 15, base_family = "serif") 
  Gr_bars = Gr_bars + theme(legend.position = "none", axis.line = element_blank(), axis.title.y = element_blank(),
          axis.ticks = element_blank(), axis.text.x = element_blank(), 
          panel.border = element_blank(), panel.margin = unit(2, "mm"), plot.margin = unit(c(0,0,0,3), "lines"))
  Gr_bars = Gr_bars + ylab(expression(paste(bold("Phases"),"",sep="")))
