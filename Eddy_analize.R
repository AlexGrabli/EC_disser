...
AllData_O$dt = cbind(AllData_O$dt, month = months[AllData_O$dt$month_number])
AllData_A_13$dt = cbind(AllData_A_13$dt, month = months[AllData_A_13$dt$month_number])
...
##For article
source(file = "Eddy_postproduction.r", local=TRUE)
AllData_O$daily_f = fread("O_13_filled_2.csv")
AllData_A_13$daily_f = fread("A_13_filled_2.csv")
#Разобраться со скользящим средним

PlotWindRoses(AllData_E_14f$dt)
PlotWindRoses(AllData_O$dt)
PlotBiomet(list(AllData_A_13, AllData_O), filled = TRUE, startDoy = 120, endDoy = 270)
PlotDiurnal(list(AllData_A_13, AllData_O), startM=4, endM=8)
PlotFluxSep(list(AllData_A_13, AllData_O), filled = TRUE, startDoy = 120, endDoy = 270)
PlotFluxSepCum(list(AllData_A_13, AllData_O), startDoy = 120, endDoy = 270, filled = TRUE)
...
library(xlsx)
write.xlsx(AllData_O$dt, file="Kursk_data.xlsx", sheetName="half_our")
write.xlsx(AllData_O$daily, file="Kursk_data.xlsx", sheetName="daily_without_manual_filling", append=TRUE)
write.xlsx(AllData_O$daily_f, file="Kursk_data.xlsx", sheetName="daily_with_manual_filling", append=TRUE)

write.csv(AllData_O$dt, file="Kursk_data_half_our.csv")
write.csv(AllData_O$daily, file="Kursk_data_daily_without_manual_filling.csv")
write.csv(AllData_O$daily_f, file="Kursk_data_daily_with_manual_filling.csv")
...
Gr_NEE_cum = ggplot() +
  geom_line(data = AllData_A_13$daily[AllData_A_13$daily[['Doy']] >114 & AllData_A_13$daily[['Doy']] <275,], aes(x=Doy, y=cumsum((NEE_f_sums * 12*18 /10000))), size=1, linetype =2, position=pd) +
  geom_line(data = AllData_O$daily[AllData_O$daily[['Doy']] >114 & AllData_O$daily[['Doy']] < 275,], aes(x=Doy, y=cumsum((NEE_f_sums * 12*18 /10000))),size=1,  position=pd) +
  #xlab("Day of the year ")+
  ylab(expression(paste(bold("Cumulative NEE")," ( g "," ",C[CO[2]]," ",m^-2," "," )",sep="")))+
  #μmol CO2 m-2s-1)")+
  scale_x_continuous(breaks = round(seq(min(AllData_A_13$daily$Doy), max(AllData_A_13$daily$Doy), by = 30),1))+
  geom_hline(yintercept = 0, linetype=2)+
  theme_few(base_size = 15, base_family = "serif")+
...
Gr_Reco_cum = ggplot() +
  geom_line(data = AllData_A_13$daily[AllData_A_13$daily[['Doy']] >114 & AllData_A_13$daily[['Doy']] <275,], aes(x=Doy, y=cumsum((Reco * 12*18 /10000))), size=1,linetype =2, position=pd) +
  geom_line(data = AllData_O$daily[AllData_O$daily[['Doy']] >114 & AllData_O$daily[['Doy']] < 275,], aes(x=Doy, y=cumsum((Reco * 12*18 /10000))),size=1,  position=pd) +
  scale_x_continuous(breaks = round(seq(min(AllData_A_13$daily$Doy), max(AllData_A_13$daily$Doy), by = 30),1))+
  #xlab("Day of the year ")+
...
Gr_GPP_cum = ggplot() +
  geom_line(data = AllData_A_13$daily[AllData_A_13$daily[['Doy']] >114 & AllData_A_13$daily[['Doy']] <275,], aes(x=Doy, y=cumsum((GPP * 12*18 /10000))), size=1, linetype =2, position=pd) +
  geom_line(data = AllData_O$daily[AllData_O$daily[['Doy']] >114 & AllData_O$daily[['Doy']] < 275,], aes(x=Doy, y=cumsum((GPP * 12*18 /10000))),size=1, position=pd) +
  scale_x_continuous(breaks = round(seq(min(AllData_A_13$daily$Doy), max(AllData_A_13$daily$Doy), by = 30),1))+
  xlab("Day of the year ")+
