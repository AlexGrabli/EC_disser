...
load_precip_2023 <- function(precip_path, year = 2023) {
  ...
  # Логика как в Weather_DONE_FINAL_AGAIN.R
  dat <- read_delim(
...
gapfilled_2023_path <- "????2_BarleyFilledAllScen_65p_biom_thrash_new2505.csv"
biomet_2023_path <- "Anal11_biomet.csv"
precip_2023_path <- "??????.csv"
...
# ----------------------- Example: daily precipitation (2016) -----------------------
library(ggplot2)
library(dplyr)
library(lubridate)

df16precip <- read.csv("2016_precip.csv", sep = ";", dec = ".")
df16precip$Date_parsed <- dmy_hm(df16precip$Date)
df16precip$Day <- as.Date(df16precip$Date_parsed)

daily_data <- df16precip %>%
  group_by(Day) %>%
  summarise(Daily_RRR = sum(RRR, na.rm = TRUE))

ggplot(daily_data, aes(x = Day, y = Daily_RRR)) +
  geom_col(fill = "steelblue", width = 1) +
  labs(title = "Daily precipitation totals",
       subtitle = "2016",
       x = "Date",
       y = "Precipitation (mm)") +
